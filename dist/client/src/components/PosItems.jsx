    ...
    return discountedItems.length > 0 ? (<Box_1.default>
      <ImageList_1.default sx={{
        ...
        {discountedItemsMap.map((item) => (<ImageListItem_1.default key={item._id}>
            <ButtonBase_1.default onClick={() => handleAddToCartWithDiscount(item._id)} disabled={addDiscountedItemToCartLoading || removeFromCartLoading} disableRipple>
              <Box_1.default>
                ...
                .map((cartItem) => (<Grid_1.default container justifyContent="center" spacing={1} maxHeight="8px" key={cartItem._id}>
                    <Grid_1.default>
                      <Typography_1.default fontSize=".7rem" sx={{ textAlign: "center" }}>
                        <Button_1.default variant="outlined" size="small" onClick={() => handleRemoveFromCartDiscount(item._id)} disabled={loading || removeFromCartLoading}>
                          Remove One
                        </Button_1.default>
